...
/**
 * Build a structured result object for a DateTime, carrying several useful
 * representations alongside the human-readable `formatted` string (which uses
 * the same preset/token the string output would).
 *
 * @param {import("luxon").DateTime} dt
 * @param {{ formatType?: ('preset'|'token'), preset?: string, token?: string, locale?: string }} [opts]
 * @returns {{ formatted: string, iso: string, epoch: number, seconds: number, zone: string, locale: string }}
 */
function buildJsonResult(dt, opts) {
  const locale = opts && opts.locale ? String(opts.locale).trim() : "";
  const d = locale ? dt.setLocale(locale) : dt;
  return {
    formatted: formatDateTime(dt, opts),
    iso: dt.toISO(),
    epoch: dt.toMillis(),
    seconds: Math.floor(dt.toSeconds()),
    zone: dt.zoneName,
    locale: d.locale,
  };
}
